...
class ArticulacionesFinalizadasExport extends FatherExport
{
  ...
  public function registerEvents(): array
  {
    ...
    return [
      AfterSheet::class => function(AfterSheet $event) use ($styles) {
        ...
        $event->sheet->mergeCells('A1:K6');
        $event->sheet->mergeCells('L1:Q5');
        $event->sheet->mergeCells('L6:Q6');
        $event->sheet->setCellValue('L6', 'Entregables');
        $event->sheet->getStyle('L6')->getAlignment()->setHorizontal(Alignment::HORIZONTAL_CENTER);
        $event->sheet->getStyle('L6:Q6')->getFont()->setSize(14)->setBold(1);
        $event->sheet->getStyle('L6:Q6')->applyFromArray($this->styleArray());
        $event->sheet->getStyle('L6:Q6')->applyFromArray($styles['pares']);
        $init = 'A';
        for ($i=0; $i < 17 ; $i++) {
          $temp = $init++;
  ...
  */
  ...
  {
    return view('exports.articulacion.grafico_nodo', [
