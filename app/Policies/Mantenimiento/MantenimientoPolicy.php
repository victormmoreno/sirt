...
class MantenimientoPolicy
{
    ...
    public function show(User $user, $mantenimiento)
    {
        if ($user->hasAnyRole([User::IsAdministrador()]) && session()->get('login_role') == User::IsAdministrador()) {
            return true;
        } elseif ($user->hasAnyRole([User::IsDinamizador()]) && session()->get('login_role') == User::IsDinamizador() && $mantenimiento->equipo->lineatecnologicanodo->nodo->id == $user->dinamizador->nodo->id) {
            return true;
        } elseif ($user->hasAnyRole([User::IsGestor()]) && session()->get('login_role') == User::IsGestor() && $mantenimiento->equipo->lineatecnologicanodo->lineatecnologica->id == $user->gestor->lineatecnologica->id) {
    ...
    {
        return (bool) $user->hasAnyRole([User::IsDinamizador()]) && session()->get('login_role') == User::IsDinamizador() && $mantenimiento->equipo->lineatecnologicanodo->nodo->id == $user->dinamizador->nodo->id;
