...
@section('content-pdf')
    ...
    <tbody>
      <tr>
        ...
        @else
          ...
          <td>{{ $actividad->actividad->gestor->user->nombres }} {{ $actividad->actividad->gestor->user->apellidos }}</td>
        @endif
      </tr>
    </tbody>
  </table>
  ...
  @else
    <table class="striped centered">
      ...
      <tbody>
        @forelse ($talentos->talentos as $value)
          <tr>
            <td>{{ $value->user->documento }}</td>
            <td>{{ $value->user->nombres }} {{ $value->user->apellidos }}</td>
            <td>{{ $value->user->email }}</td>
  ...
  <table class="striped centered">
    ...
    <tbody>
      @if ($tipo_actividad == 'proyecto')
        ...
        @endforelse
      @else
            @forelse ($usos->actividad->usoinfraestructuras as $value)
            <tr>
              <td>{{ $value->fecha->isoFormat('YYYY-MM-DD') }}</td>
              <td>{{ $value->usogestores->sum('pivot.asesoria_directa') }}</td>
              <td>{{ $value->usogestores->sum('pivot.asesoria_indirecta') }}</td>
  ...
  <div class="row">
    ...
    <div class="column">
      ...
      @else
      <small>Firma del talento interlocutor - {{$actividad->talentos()->wherePivot('talento_lider', '=', 1)->first()->user->nombres}} {{$actividad->talentos()->wherePivot('talento_lider', '=', 1)->first()->user->apellidos}}</small>
      @endif
    </div>
  </div>
  <br>
