                                    ...
                                    <td>
                                        <p class="p-v-xs">
                                            @if (isset($gexistente[$gestor->gestor->id]))
                                                <input class="filled-in" type="checkbox" name="gestores[]" checked
                                                    value="{{ $gestor->gestor->id }}"
                                                    id="gestor-{{ $gestor->gestor->id }}"
                                                    onchange="addGestorComite2(this.value)">
                                            @else
                                                <input class="filled-in" type="checkbox" name="gestores[]"
                                                    value="{{ $gestor->gestor->id }}"
                                                    id="gestor-{{ $gestor->gestor->id }}"
                                                    onchange="addGestorComite2(this.value)">
                                            @endif
                                            <label
                                                for="gestor-{{ $gestor->gestor->id }}">{{ $gestor->nombre }}</label>
                                        </p>
                                    </td>
                                    <td>
                                        @if (isset($gexistente[$gestor->gestor->id]))
                                            <input value="{{ $gexistente[$gestor->gestor->id]['inicio'] }}"
                                                id="txthorainiciogestor-{{ $gestor->gestor->id }}" type="text"
                                                class="pickertime black-text" name="horas_inicio[]"
                                                placeholder="Desde">
                                        @else
                                            <input disabled id="txthorainiciogestor-{{ $gestor->gestor->id }}"
                                                type="text" class="pickertime black-text" name="horas_inicio[]"
                                                placeholder="Desde">
                                        @endif
                                        <small id="txthorainiciogestor-{{ $gestor->gestor->id }}-error"
                                            class="error red-text"></small>
                                    </td>
                                    <td>
                                        @if (isset($gexistente[$gestor->gestor->id]))
                                            <input value="{{ $gexistente[$gestor->gestor->id]['fin'] }}"
                                                id="txthorafingestor-{{ $gestor->gestor->id }}" type="text"
                                                class="pickertime text-black" name="horas_fin[]" placeholder="Hasta">
                                        @else
                                            <input disabled id="txthorafingestor-{{ $gestor->gestor->id }}"
                                                type="text" class="pickertime text-black" name="horas_fin[]"
                                                placeholder="Hasta">
                                        @endif
                                        <small id="txthorafingestor-{{ $gestor->gestor->id }}-error"
                                            class="error red-text"></small>
                                    </td>
                                </tr>
                            @endforeach
