...
# Encuesta de Satisfaccion {{class_basename($query)}} | {{$query->codigo_proyecto}}.

Señor(a)<br>
<b>_{{$notifiable->nombres.' '.$notifiable->apellidos}}_</b><br>
Cordial Saludo.
<br>
Ha recibido este mensaje porque se solicitó diligenciar la encuesta de satisfaccion de {{class_basename($query)}} {{$query->codigo_proyecto}}.

@component('mail::promotion')
    <center>
        <h3 class="subtittle">{{class_basename($query)}}</h3>
        <h3 class="subtittle-value">{{$query->codigo_proyecto}} - {{$query->nombre}}</h3>
    </center>
@endcomponent
