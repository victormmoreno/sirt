                    ...
                    <div class="card">
                        <div class="card-content">
                            <div class="row">
                                ...
                                <br />
                                <form action="{{ route('idea.asignar.experto', $idea) }}" method="POST"
                                    name="frmUpdateGestorIdea">
                                    {!! method_field('PUT') !!}
                                    @csrf
                                    <div class="row">
                                        <h5 class="center">Asignar el experto asesor a la idea: <b>{{ $idea->codigo_idea }}
                                        ...
                                        <div class="input-field col s12 m6 l6 offset-m3 offset-l3">
                                            <select id="txtgestor_id" class="js-states" name="txtgestor_id"
                                                ...
                                                @forelse ($gestores as $id => $gestor)
                                                    <option value="{{ $gestor->gestor_id }}"
                                                        {{ $gestor->gestor_id == $idea->gestor_id ? 'selected' : '' }}
                                                        {{ old('txtgestor_id') == $gestor->gestor_id ? 'selected' : '' }}>
                                                        {{ $gestor->nombres_gestor }}</option>
                                                @empty
                                    ...
                                    <center>
                                        ...
                                        <a href="{{ route('idea.detalle', $idea->id) }}"
