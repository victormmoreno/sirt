            ...
            <div class="center">
                ...
                <div class="row">
                    <div class="input-field col s12 m6 l6">
                        @forelse($tipotalentos as  $id => $nombre)
                        <p align="left" class="p-v-xs">
                        @if(isset($user->talento->tipo_talento_id))
                            <input  class="tipotalento" id="tipotalento_{{$id}}" name="txttipotalento" type="radio"  value="{{$id}}" {{ old("txttipotalento", $user->talento->tipo_talento_id) == $id ? 'checked':'' }} {{session()->get('login_role') == App\User::IsAdministrador() || session()->get('login_role') == App\User::IsDinamizador()  ? 'readonly' : ''}} {{session()->get('login_role') == App\User::IsAdministrador() || session()->get('login_role') == App\User::IsDinamizador()  ? 'onclick=this.checked=!this.checked;' : '' }}/>
                        @else
                        <input  class="tipotalento" id="tipotalento_{{$id}}" name="txttipotalento" type="radio" value="{{$id}}" {{session()->get('login_role') == App\User::IsAdministrador() || session()->get('login_role') == App\User::IsDinamizador()  ? 'onclick=this.checked=!this.checked;' : '' }}/>
                        @endif

                            <label  for="tipotalento_{{$id}}">
                                {{$nombre}}
                            </label>
                        </p>
                        @empty
                            <p>no se econtraron tipos de talentos</p>

                        @endforelse
                        <small id="txttipotalento-error"  class="error red-text"></small>

                    </div>
                    <div class="input-field col s12 m6 l6 seccionInfoTipoTalento">
                        <div class="valign-wrapper selecttipotalento" style="display:block">
                            <h5>< Selecciona un tipo de talento</h5>
                        </div>
                        <div class="row " >
                            <div class="input-field col s12 m12 l12 aprendizSena egresadoSena instructorSena funcionarioSena" style="display:none" >
                                <select class="txtregional" id="txtregional" name="txtregional" style="width: 100%" tabindex="-1" onchange="tipoTalento.getCentroFormacion()" {{session()->get('login_role') == App\User::IsAdministrador() || session()->get('login_role') == App\User::IsDinamizador()  ? 'disabled' : ''}}>
                                ...
                                <small id="txtregional-error"  class="error red-text"></small>
                            </div>
                            <div class="input-field col s12 m12 l12 aprendizSena egresadoSena instructorSena funcionarioSena" style="display:none">

                                <select class="centros" id="txtcentroformacion" name="txtcentroformacion" style="width: 100%" tabindex="-1" {{session()->get('login_role') == App\User::IsAdministrador() || session()->get('login_role') == App\User::IsDinamizador()  ? 'disabled' : ''}}>
                                ...
                                <small id="txtcentroformacion-error"  class="error red-text"></small>
                            </div>
                            <div class="input-field col s12 m12 l12 aprendizSena egresadoSena " style="display:none" >
                                ...
                                <small id="txtprogramaformacion-error"  class="error red-text"></small>

                            </div>
                        </div>
                        <div class="row ">
                            <div class="input-field col s12 m12 l12 egresadoSena" style="display:none" >
                        ...
                        <div class="row ">
                            <div class="input-field col s12 m12 l12 funcionarioSena" style="display:none">

                                <input class="validate" id="txtdependencia" name="txtdependencia" type="text"  value="{{ isset($user->talento->programa_formacion) ? $user->talento->programa_formacion : old('txtdependencia')}}" {{session()->get('login_role') == App\User::IsAdministrador() || session()->get('login_role') == App\User::IsDinamizador()  ? 'readonly' : ''}}>
                        ...
                        <div class="row universitario" style="display:none">
                            <div class="input-field col s12 m12 l12" >
                                <select class="" id="txttipoestudio" name="txttipoestudio" style="width: 100%" tabindex="-1" {{session()->get('login_role') == App\User::IsAdministrador() || session()->get('login_role') == App\User::IsDinamizador()  ? 'disabled' : ''}}>
                                        <option value="">Seleccione Tipo Estudio</option>
                                        @foreach($tipoestudios as $id => $nombre)
                                            @if(isset($user->talento->entidad->centro->regional->id))
                                            <option value="{{$id}}" {{old('txttipoestudio',$user->talento->entidad->centro->regional->id) ==$id ? 'selected':''}}>{{$nombre}}</option>
                                            @else
                            ...
                            <div class="input-field col s12 m12 l12" >
                                <input class="validate" id="txtuniversidad" name="txtuniversidad" type="text"  value="{{ isset($user->talento->programa_formacion) ? $user->talento->programa_formacion : old('txtuniversidad')}}" {{session()->get('login_role') == App\User::IsAdministrador() || session()->get('login_role') == App\User::IsDinamizador()  ? 'readonly' : ''}}>
                            ...
                            <div class="input-field col s12 m12 l12" >
                                <input class="validate" id="txtcarrera" name="txtcarrera" type="text"  value="{{ isset($user->talento->programa_formacion) ? $user->talento->programa_formacion : old('txtcarrera')}}" {{session()->get('login_role') == App\User::IsAdministrador() || session()->get('login_role') == App\User::IsDinamizador()  ? 'readonly' : ''}}>
                        ...
                        <div class="row funcionarioEmpresa" style="display:none">
                            ...
                            <div class="input-field col s12 m12 l12" >
                                <input class="validate" id="txtempresa" name="txtempresa" type="text"  value="{{ isset($user->talento->programa_formacion) ? $user->talento->programa_formacion : old('txtempresa')}}" {{session()->get('login_role') == App\User::IsAdministrador() || session()->get('login_role') == App\User::IsDinamizador()  ? 'readonly' : ''}}>
