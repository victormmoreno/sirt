    ...
    <div class="input-field col s12 m6 l6">
        ...
        <select class="" id="txttipo_documento" name="txttipo_documento" style="width: 100%" tabindex="-1">
            ...
            @foreach($tiposdocumentos as $value)
                <option value="{{$value->id}}" {{ old('txttipo_documento') == $value->tipodocumento_id ? 'selected':'' }}>{{$value->nombre}}</option> 
            @endforeach
        </select>
...
<div class="row">
    <div class="input-field col s12 m6 l6">
        ...
        <input class="validate" id="txtfecha_nacimiento" name="txtfecha_nacimiento" type="text" value="{{ isset($user->fechanacimiento) ? $user->fechanacimiento : old('txtfecha_nacimiento')}}">
    ...
    <div class="input-field col s12 m6 l6">
        ...
        <select class="" id="txtestrato" name="txtestrato" style="width: 100%" tabindex="-1">
            ...
            @for($i =1; $i <= 6; $i++)
                <option value="{{ $i }}"  {{ isset($user->estrato) == 3 ? 'selected' : ''}}>{{$i}}</option> 
            @endfor
        </select>
...
<div class="row">
    <div class="input-field col s12 m6 l6">
        ...
        <input class="validate" id="txtemail" name="txtemail" type="email" value="{{ isset($user->email) ? $user->email : old('txtapellidos')}}">
                ...
                <div class="input-field col s12 m6 l6 offset-l3 m3 s3">
                    ...
                    <select class="" id="txtgrado_escolaridad" name="txtgrado_escolaridad" style="width: 100%" tabindex="-1">
                        ...
                        @foreach($gradosescolaridad as $value)
                            <option value="{{$value->id}}" {{ old('txtgrado_escolaridad') == $value->gradoescolaridad_id ? 'selected':'' }}>{{$value->nombre}}</option> 
                        @endforeach
                    </select>
                    <label for="txtcelular">Grado Escolaridad*</label>
