<div class="mailbox-view-header">
    <div class="left">
        <div class="left">
            ...
            <span class="mailbox-author">{{$accompaniment->present()->accompanimentInterlocutorTalent()}} ({{__('Interlocutory talent')}})
                @can('update', $accompaniment)
                    <a href="{{ route('accompaniments.changeinterlocutor', $accompaniment) }}" class="orange-text text-darken-2 pointer tooltipped" data-position="right" data-tooltip="cambiar {{__('Interlocutory talent')}}"><i class="tiny material-icons">edit</i></a>
                @endcan
            </span>
        </div>
    </div>
    <div class="right mailbox-buttons">
        ...
        @elseif(auth()->user()->can('update', $accompaniment))
            <a href="{{route('accompaniments.requestapproval', [$accompaniment])}}" class="waves-effect waves-orange btn orange m-t-xs">Enviar solicitud de aprobación al talento</a>
        @else
