<form action="{{route('articulations.manage-approval', [$articulation->id, $articulation->phase_id == \App\Models\Fase::IsCierre() ? 'Finalizado' : 'Cierre'])}}" method="POST" style="width: 100%" id="frmApprovalArticulations">
    ...
    @if(isset($ult_notificacion))
        ...
        <button type="submit" onclick="endorsementQuestionArticulation(event)" class="center-align center waves-effect waves-light btn orange btn-large modal-trigger" style="margin-bottom: 10px; margin-right: 10px; margin-top: 10px; width: 100%;">
            <i class="material-icons left">done</i>
            Aprobar aval
        </button>
        <button type="submit" onclick="questionRejectEndorsementArticulation(event)" class="center-align waves-effect waves-light btn red modal-trigger" style="margin-bottom: 10px; margin-right: 10px; margin-top: 0px; width: 100%">
